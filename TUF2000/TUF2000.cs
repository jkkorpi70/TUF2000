...
namespace TUF2000MainForm
{
    class TUF2000
    {
        ...
        //------------------------------------------------------------------------------
        // Extract low or high byte from integer
        //------------------------------------------------------------------------------
        ...
        //------------------------------------------------------------------------------
        // Convert byte to BCD numbers.
        // Tens -> move high byte 4 right, AND with 0000 1111, and multiplied by 10.
        // Finally low byte is added.
        //------------------------------------------------------------------------------
        private int IntToBCD(int SourceValue) { return ((SourceValue >> 4) & 0x0f) * 10 + (SourceValue & 0x0f); }

        //------------------------------------------------------------------------------
        // Convert given value to date and time using function above
        //------------------------------------------------------------------------------
        private string GetDateTime(int value1, int value2, int value3) // Ei näin vielä bytet / 2
        {
            string DateTime = Convert.ToString(IntToBCD(GetHighByte(value3))) + "-"; // Year
            DateTime += Convert.ToString(IntToBCD(GetLowByte(value3))) + "-";       // Month
            DateTime += Convert.ToString(IntToBCD(GetHighByte(value2))) + " ";      // Day
            DateTime += Convert.ToString(IntToBCD(GetLowByte(value2))) + ":";       // Hour
            DateTime += Convert.ToString(IntToBCD(GetHighByte(value1))) + ":";      // Min
            DateTime += Convert.ToString(IntToBCD(GetLowByte(value1)));             // Sec
